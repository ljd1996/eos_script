...
create_wallet_node () {
  ...
  cleos --wallet-url http://${WALLET_HOST}:${WALLET_PORT} wallet create --file $WALLET_DIR/wpk/default.wpk 2>/dev/null 
  cleos --wallet-url http://${WALLET_HOST}:${WALLET_PORT} wallet import --private-key ${OWNER_KEY} 2>/dev/null
  # cleos --wallet-url http://${WALLET_HOST}:${WALLET_PORT} wallet create -n ${ACCOUNT_CREATOR} --file $WALLET_DIR/wpk/${ACCOUNT_CREATOR}.wpk 2>/dev/null 
  # cleos --wallet-url http://${WALLET_HOST}:${WALLET_PORT} wallet import -n --private-key ${ACCOUNT_CREATOR} ${CREATOR_PRIV_KEY} 2>/dev/null
  # cleos --wallet-url http://${WALLET_HOST}:${WALLET_PORT} wallet create -n eosio --file $WALLET_DIR/wpk/eosio.wpk
  # cleos --wallet-url http://${WALLET_HOST}:${WALLET_PORT} wallet create -n ${ACCOUNT_CREATOR} --file $WALLET_DIR/wpk/${ACCOUNT_CREATOR}.wpk
}
...
unlock_proc () {
  for INF in $(ls $WALLET_DIR/wpk/*.wpk);do
    ...
    cleos --wallet-url http://$WALLET_HOST:$WALLET_PORT wallet unlock -n $_UID --password=$(cat $INF | tail -n 1 | sed "s/\"//g") >/dev/null
  done
}
unlock_all () {
  for((x=0;x<${#WLT[@]};x++));do
...
create_account () {
  ...
  [ ! -d $KEY_DIR ] && mkdir $KEY_DIR
  cleos create key --file $KEY_DIR/$2.key
  ...
  for((x=0;x<${#WLT[@]};x++));do
    ...
    CMD="cleos --wallet-url http://${WALLET_HOST}:${WALLET_PORT} -u http://$HTTP_HOST:$HTTP_PORT"

    CHK=$($CMD get account $1 | grep "owner" | grep "EOS" |wc -l )
    ...
    CHK=$($CMD get account $2 | grep "owner" | grep "EOS" | wc -l )
    ...
    $CMD wallet create -n $2 --file $WALLET_DIR/wpk/$2.wpk
    $CMD wallet import -n $2 --private-key $PRIV > /dev/null 2>&1
    [ $? -eq 0 ] && echo_s || echo_f
  done

  $CMD create account $1 $2 $PUB $PUB

  echo "-----------------------------------------"
  # $CMD push action eosio.token transfer '["'$1'","'$2'","'$CREATE_AMOUNT' '$CURRENCY'","Initial coin"]' -p $1 > /dev/null 2>&1
  $CMD push action eosio.token transfer '["'$1'","'$2'","'$CREATE_AMOUNT' '$CURRENCY'","Initial coin"]' -p $1@active
...
result_job () {
  cat nohup.out | grep "executed transaction:" > job/transaction_ret.log
  pd_rnd=$[$RANDOM % ${#PDINFO[@]}]
  eval $(awk -F"|" '{print "PNAME="$1" HTTP_HOST="$2" HTTP_PORT="$3}' <<< ${PDINFO[$pd_rnd]})
